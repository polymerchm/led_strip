...
void start_buffer_build(uint8_t buffer_index)
{
    ...
    // int pulse_duration_frames = random_in_range(CONFIG_SPARKLE_PULSE_DURATION_FRAMES+20,
    //                             CONFIG_SPARKLE_PULSE_DURATION_FRAMES-20, &idum
    //                             );
    // if (pulse_duration_frames % 2 == 0) pulse_duration_frames++; // must be odd

    // range of time frames ( corrected for leadig or trailing edge of the pulses)
    // uint16_t min_frame = (pulse_duration_frames - 1) / 2;
    ...
    // uint8_t std_dev_min = (int) pulse_duration_frames / 10.0,
    //         std_dev_max = (int) pulse_duration_frames / 6.0;
    uint8_t next_level;
    for (int i = 0; i < num_to_pulse; i++)
    {
        ...
        uint16_t min_frame = (pulse_duration_frames - 1) / 2;
        uint16_t max_frame = CONFIG_SPARKLE_TIME_FRAMES - min_frame - 1;
        if (pulse_duration_frames % 2 == 0)
            pulse_duration_frames++; // must be odd
        ...
        pulse_max = random_in_range(255, 60, &idum);
        uint8_t std_dev_min = (int)pulse_duration_frames / 10.0,
                std_dev_max = (int)pulse_duration_frames / 6.0; // between 255 and 60 (100% and 24% max intensity)
        std_dev = random_in_range(std_dev_max, std_dev_min, &idum);
        for (uint16_t frame_index = 0; frame_index < pulse_duration_frames; frame_index++)
        {
